<div align="center">
	<h1>
		<img src="https://media.tenor.com/EVg-zVvNcu8AAAAi/bulbizarre.gif"
		alt="Bulbasur Gif Pixel Art" width="50">
		DAW116 | Bases de Datos
	</h1>
</div>

<br>

<div>
	<h2>
		<img src="https://media.tenor.com/uj-429a6v-YAAAAi/pixel-art-gmail.gif"
		alt="Signo exclamación e interrogación" width="20">
		Información del repositorio.
	</h2>
	<p>Repositorio de DAW116 | Irene Rodríguez, en el que encontrarás información
		sobre las diferentes prácticas y temario de la asignatura Bases de Datos(BDD) del curso 2024-2025.
	</p>
</div>

<br>

<div align="center">
	<img src="https://media.tenor.com/nL6vKOBztP0AAAAj/aesthetic-green.gif" alt="Divider de hojas y plantitas"
	width="350">
</div>

<br>

<div>
	<h2>
		<img src="https://media.tenor.com/uj-429a6v-YAAAAi/pixel-art-gmail.gif"
		alt="Signo exclamación e interrogación" width="20">
		Contenidos.
	</h2>
	<p>
		En él están las prácticas y tareas correspondientes a cada unidad didáctica
		vista en clase.
	</p>
	<p>
		Los contenidos se encuentran organizados en la siguiente tabla por temas,
		la explicación y la práctica realizada.
	</p>
	<p>
		En la tabla también se indica el estado en el que se encuentra la tarea:
  		<li>Completada ✅</li>
		<li>Pendiente de subida❌</li>
		<li>Pendiente de finalización❓</li>
	</p>
	<table>
		<thead>
			<tr>
				<th>
					Tema
				</th>
				<th>
					Explicación del tema
				</th>
				<th>
					Práctica
				</th>
				<th>
					Contenido de la práctica
				</th>
				<th>
					Estado (✅, ❌,❓)
				</th>
				<th>Última revisión</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td rowspan="4">
					Tema 1. Fundamentos de las bases de datos.
				</td>
				<td rowspan="4">
					Introducción a los sistemas de almacenamiento y tipos de bases de datos.
				</td>
				<td>
					Actividad 1: Infografía comparativa de modelos de bases de datos
				</td>
				<td>
					Comparativa de los diferentes modelos de BD
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 2: Cuadro comparativo de Sistemas Gestores de Bases de Datos.
				</td>
				<td>
					Comparativa entre los diferentes sistemas gestores de BD
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 3: Aplicación práctica de tipos de ficheros
				</td>
				<td>
					Planteamiento de varios escenarios y búsqueda del SGBD óptimo
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 4: Big Data y bases de datos distribuidas
				</td>
				<td>
					Enfoque de Big Data y beneficios del uso de las BD distribuídas.
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
            <tr>
				<td rowspan="4">
					Tema 2. Diseño lógico de bases de datos.
				</td>
				<td rowspan="4">
					Introducción al modelo Entidad-Relación y su transformación al modelo relacional.
				</td>
				<td>
					Actividad 1: Diseño de un diagrama Entidad-Relación básico
				</td>
				<td>
					Creación de un diagrama E/R con entidades, atributos y relaciones simples
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 2: Diseño E/R ampliado con jerarquías y relaciones especiales
				</td>
				<td>
					Uso de especializaciones, generalizaciones y relaciones complejas
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 3: Paso a modelo relacional
				</td>
				<td>
					Transformación del modelo E/R a tablas del modelo relacional
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 4: Aplicación de reglas de conversión y normalización
				</td>
				<td>
					Aplicación de reglas de conversión y formas normales básicas
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
            <tr>
				<td rowspan="4">
					Tema 3. Bases de datos relacionales.
				</td>
				<td rowspan="4">
					Introducción al uso práctico de MySQL y gestión básica de bases de datos.
				</td>
				<td>
					Actividad 1: Instalación y configuración de MySQL
				</td>
				<td>
					Instalación del SGBD MySQL y configuración inicial del entorno
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 2: Creación de la base de datos empresa
				</td>
				<td>
					Diseño y creación de la base de datos empresa con sus tablas
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 3: Modificación y optimización de la base de datos empresa
				</td>
				<td>
					Alteración de tablas, optimización de estructuras y mejoras del diseño
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 4: Gestión de usuarios y permisos
				</td>
				<td>
					Creación de usuarios, asignación de permisos y control de accesos
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
            <tr>
				<td rowspan="4">
					Tema 4. Realización de consultas
				</td>
				<td rowspan="4">
					Elaboración de consultas SQL para la recuperación y análisis de información en bases de datos.
				</td>
				<td>
					Actividad 1: Consultas simples con condiciones y ordenación
				</td>
				<td>
					Uso de SELECT con WHERE, ORDER BY y filtros básicos
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 2: Consultas multitabla con JOIN y LEFT JOIN
				</td>
				<td>
					Consultas combinando varias tablas mediante JOIN y LEFT JOIN
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 3: Consultas con funciones de agregado y agrupación
				</td>
				<td>
					Uso de funciones COUNT, SUM, AVG, GROUP BY y HAVING
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 4: Vistas, subconsultas y análisis de ejecución
				</td>
				<td>
					Creación de vistas, uso de subconsultas y análisis con EXPLAIN
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
            <tr>
				<td rowspan="4">
					Tema 5. Manipulación y transacciones en bases de datos
				</td>
				<td rowspan="4">
					Manipulación de datos y control de transacciones para garantizar la consistencia e integridad de la información.
				</td>
				<td>
					Actividad 1: Manipulación de datos
				</td>
				<td>
					Operaciones INSERT, UPDATE y DELETE sobre tablas
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 2: Validación de la integridad referencial
				</td>
				<td>
					Uso de claves primarias y foráneas para mantener la integridad de los datos
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 3: Transacciones y control de errores
				</td>
				<td>
					Gestión de transacciones con COMMIT, ROLLBACK y SAVEPOINT
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 4: Concurrencia, bloqueo y propiedades ACID
				</td>
				<td>
					Control de concurrencia, bloqueos y principios ACID
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td rowspan="4">
					Tema 6. Programación de bases de datos
				</td>
				<td rowspan="4">
					Programación en el SGBD mediante procedimientos, funciones y otros elementos avanzados.
				</td>
				<td>
					Actividad 1: Procedimiento almacenado
				</td>
				<td>
					Creación y uso de procedimientos almacenados en MySQL
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 2: Función almacenada
				</td>
				<td>
					Definición y uso de funciones almacenadas
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 3: Cursor
				</td>
				<td>
					Uso de cursores para el tratamiento de conjuntos de resultados
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 4: Trigger (Disparador)
				</td>
				<td>
					Creación de triggers para automatizar acciones en la base de datos
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td rowspan="4">
					Tema 7. Bases de datos no relacionales
				</td>
				<td rowspan="4">
					Introducción a las bases de datos NoSQL y uso práctico de MongoDB.
				</td>
				<td>
					Actividad 1: Análisis y aplicación de bases de datos NoSQL
				</td>
				<td>
					Estudio de los distintos tipos de bases de datos NoSQL y sus casos de uso
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 2: Instalación y documentación de MongoDB
				</td>
				<td>
					Instalación del SGBD MongoDB y documentación del proceso
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 3: Creación y consulta de datos en MongoDB
				</td>
				<td>
					Inserción y consultas de documentos en colecciones MongoDB
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>
					Actividad 4: Actualización y eliminación de datos en MongoDB
				</td>
				<td>
					Actualización y borrado de documentos mediante operaciones NoSQL
				</td>
				<td>❌</td>
				<td>---</td>
			</tr>
		</tbody>
	</table>
</div>

<br>

<div align="center">
	<img src="https://media.tenor.com/nL6vKOBztP0AAAAj/aesthetic-green.gif" alt="Divider de hojas y plantitas"
	width="350">
</div>

<br>

<div>
	<h2>
		<img src="https://media.tenor.com/uj-429a6v-YAAAAi/pixel-art-gmail.gif"
		alt="Signo exclamación e interrogación" width="20">
		Exámenes y prácticas de examen
	</h2>
	<p>
		Aquí están otros contenidos relacionados con el repositorio, pero que
		se han incluido posteriormente a la creación de este.
	</p>
	<table>
		<thead>
			<tr>
				<th>
					Examen
				</th>
				<th>
					Explicación del contenido
				</th>
				<th>
					Carpeta
				</th>
				<th>
					Fecha realización
				</th>
				<th>
					Estado (✅, ❌,❓)
				</th>
				<th>
					Fecha de última revisión
				</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td>01</td>
				<td>---</td>
				<td>---</td>
				<td>---</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>02</td>
				<td>---</td>
				<td>---</td>
				<td>---</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>03</td>
				<td>---</td>
				<td>---</td>
				<td>---</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>Final</td>
				<td>---</td>
				<td>---</td>
				<td>---</td>
				<td>❌</td>
				<td>---</td>
			</tr>
			<tr>
				<td>Extraordinario</td>
				<td>---</td>
				<td>---</td>
				<td>---</td>
				<td>❌</td>
				<td>---</td>
			</tr>
		</tbody>
	</table>
</div>

<br>

<div align="center">
	<img src="https://media.tenor.com/nL6vKOBztP0AAAAj/aesthetic-green.gif" alt="Divider de hojas y plantitas"
	width="350">
</div>

<br>

<div align="center">
	<h2>
		<img src="https://media.tenor.com/u7Di77AeSXAAAAAi/bulbasaur-rolling.gif"
		alt="Bulbasur girando" width="50">
		Gracias por leer.
	</h2>
</div>

<br>

<div align="right">
	<p>
		Última modificación:
		<i>
			10 de enero de 2026
		</i>
	</p>
</div>
